...
{
    MSGSTRUCT msg;
    msg.uMsg = MSG_DROPDOWN;
    msg.color = stepPut.color;
    msg.x = stepPut.x;
    msg.y = stepPut.y;
    m_pTable->m_clientSocket.Send( (LPCVOID)&msg, sizeof( MSGSTRUCT ) );
}
void CTwoGame::ReceiveMsg( MSGSTRUCT *pMsg )
{
    int nRet = m_pTable->m_clientSocket.Receive( pMsg, sizeof( MSGSTRUCT ) );
